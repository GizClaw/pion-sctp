...
func waitForResetEvent(t *testing.T, bridge *test.Bridge, events <-chan uint16) uint16 {
	t.Helper()
...
func testLateStreamResetResponseKeepsReusedStreamDeliverable(t *testing.T, interleaving bool) { //nolint:cyclop
	...
	clientCompleted := make(chan uint16, 1)
	...
	require.Equal(t, streamID, waitForResetEvent(t, bridge, clientCompleted))
